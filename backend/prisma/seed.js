...
async function main() {
    ...
    const getPriorityId = (name) => priorityRecords.find((p) => p.name === name)?.id ?? priorityRecords[0].id;
    ...
    const getStatusId = (name) => statusRecords.find((status) => status.name === name)?.id ?? statusRecords[0].id;
    const getTypeId = (name) => ticketTypes.find((type) => type.name === name)?.id ?? ticketTypes[0].id;
